        ...
        return -1;
            ...
            if (strcmp(ImgType, "rgb") == 0) {
                ...
                */
                ...
                char *rgb = img;
                Log("npixel = %i\n",nPixel);
                for (Pixel = 0; Pixel < nPixel*3; Pixel++) {
                    if(rgb[Pixel+0]+rgb[Pixel+1]+rgb[Pixel+2] > 150)
                        Count++;
                Pixel = Pixel+3;
                }
                //Log("random pixel red double = %d\n",rgb[16517]);
                //Log("random pixel red float = %f\n",rgb[16517]);
                //Log("random pixel red char = %s\n",rgb[16517]);
                RSDSIF.Count = Count;
                if (Count > nPixel*3*0.25)
                    Saturation = 1;
                //RSDSIF.MinDepth = f_img[MinDepthPixel];
                RSDSIF.Saturation = Saturation;
                if (RSDScfg.Verbose) {
...
RSDS_Init(void)
{
	...
	DDefInt  (NULL, "RSDS.CameraSaturation",  "",  &RSDSIF.Saturation,   DVA_None);
    DDefInt  (NULL, "RSDS.Count",  "",  &RSDSIF.Count,   DVA_None);																			   
}
