...
extern const char SRM_1_b4_LibIdent[];
const char SRM_1_b4_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_SRM_1_b4 *rtm)
{
...
static void
SRM_1_b4_DeclQuants (void *MP)
{
    rtModel_SRM_1_b4 *rtm = (rtModel_SRM_1_b4 *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(SRM_1_b4_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(SRM_1_b4_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'",
		    Modelname, SRM_1_b4_DictDefines[i].Name);
	}
...
/*
 * SRM_1_b4_SetParams() will be invoked indirectly by the generated
 * model C code each time SRM_1_b4_New() is called.
 */
void
SRM_1_b4_SetParams (rtModel_SRM_1_b4 *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
{
    rtModel_SRM_1_b4 *rtm = (rtModel_SRM_1_b4 *)MP;
    ExternalInputs_SRM_1_b4  *in  = (ExternalInputs_SRM_1_b4 *) rtmGetU(rtm);
...
static void *
SRM_1_b4_New (struct tInfos *Inf, struct tSteeringCfgIF *CfgIF, const char *KindKey)
{
    rtModel_SRM_1_b4 *rtm;
    ...
    MatSupp_ResetQuants(SRM_1_b4_DictDefines);
...
static int
SRM_1_b4_Calc (void *MP, tSteeringIF *IF, double dt)
{
    rtModel_SRM_1_b4 *rtm = (rtModel_SRM_1_b4 *)MP;
    int osCount=0;
    ExternalInputs_SRM_1_b4  *in  = (ExternalInputs_SRM_1_b4 *) rtmGetU(rtm);
    ExternalOutputs_SRM_1_b4 *out = (ExternalOutputs_SRM_1_b4 *)rtmGetY(rtm);
...
SRM_1_b4_Register (void)
{
    ...
    m.Steering.DeclQuants		= SRM_1_b4_DeclQuants;
    m.Steering.New			= SRM_1_b4_New;
    m.Steering.Calc			= SRM_1_b4_Calc;
    m.Steering.Delete			= SRM_1_b4_Delete;
