...
extern const char VehControl_LibIdent[];
const char VehControl_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_VehControl *rtm)
{
...
static void
VehControl_DeclQuants (void *MP)
{
    rtModel_VehControl *rtm = (rtModel_VehControl *)MP;

    /*Log("%s_DeclQuants()\n", Modelname);*/

    if (rtm == NULL) {
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(VehControl_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	MatSupp_DeclQuants(VehControl_DictDefines);
...
/*
 * VehControl_SetParams() will be invoked indirectly by the generated
 * model C code each time VehControl_New() is called.
 */
void
VehControl_SetParams (rtModel_VehControl *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
static void *
VehControl_New (struct tInfos *Inf, const char *KindKey)
{
    rtModel_VehControl *rtm;
    ...
    MatSupp_ResetQuants(VehControl_DictDefines);
...
static int
VehControl_Calc (void *MP, double dt)
{
    rtModel_VehControl *rtm = (rtModel_VehControl *)MP;
    int osCount=0;
    ExternalInputs_VehControl  *in  = (ExternalInputs_VehControl *) rtmGetU(rtm);
    ExternalOutputs_VehControl *out = (ExternalOutputs_VehControl *)rtmGetY(rtm);
...
VehControl_Register (void)
{
    ...
    m.VehicleControl.DeclQuants		= VehControl_DeclQuants;
    m.VehicleControl.New		= VehControl_New;
    m.VehicleControl.Calc		= VehControl_Calc;
    m.VehicleControl.Delete		= VehControl_Delete;
