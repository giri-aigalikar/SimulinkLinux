...
#include "PoC_ACC_LCA2.h"
#include "PoC_ACC_LCA2_wrap.h"
...
extern const char PoC_ACC_LCA2_LibIdent[];
const char PoC_ACC_LCA2_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_PoC_ACC_LCA2 *rtm)
{
...
static void
PoC_ACC_LCA2_DeclQuants (void *MP)
{
    rtModel_PoC_ACC_LCA2 *rtm = (rtModel_PoC_ACC_LCA2 *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(PoC_ACC_LCA2_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(PoC_ACC_LCA2_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'",
		    Modelname, PoC_ACC_LCA2_DictDefines[i].Name);
	}
...
/*
 * PoC_ACC_LCA2_SetParams() will be invoked indirectly by the generated
 * model C code each time PoC_ACC_LCA2_New() is called.
 */
void
PoC_ACC_LCA2_SetParams (rtModel_PoC_ACC_LCA2 *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
static void *
PoC_ACC_LCA2_New (struct tInfos *Inf, const char *KindKey)
{
    rtModel_PoC_ACC_LCA2 *rtm;
    ...
    MatSupp_ResetQuants(PoC_ACC_LCA2_DictDefines);
...
static int
PoC_ACC_LCA2_Calc (void *MP, double dt)
{
    rtModel_PoC_ACC_LCA2 *rtm = (rtModel_PoC_ACC_LCA2 *)MP;
    int osCount=0;
    ExternalInputs_PoC_ACC_LCA2  *in  = (ExternalInputs_PoC_ACC_LCA2 *) rtmGetU(rtm);
    ExternalOutputs_PoC_ACC_LCA2 *out = (ExternalOutputs_PoC_ACC_LCA2 *)rtmGetY(rtm);
...
PoC_ACC_LCA2_Register (void)
{
    ...
    m.VehicleControl.DeclQuants		= PoC_ACC_LCA2_DeclQuants;
    m.VehicleControl.New		= PoC_ACC_LCA2_New;
    m.VehicleControl.Calc		= PoC_ACC_LCA2_Calc;
    m.VehicleControl.Delete		= PoC_ACC_LCA2_Delete;
