...
#include "PoC_ACC_LCA.h"
#include "PoC_ACC_LCA_wrap.h"
...
extern const char PoC_ACC_LCA_LibIdent[];
const char PoC_ACC_LCA_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_PoC_ACC_LCA *rtm)
{
...
static void
PoC_ACC_LCA_DeclQuants (void *MP)
{
    rtModel_PoC_ACC_LCA *rtm = (rtModel_PoC_ACC_LCA *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(PoC_ACC_LCA_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(PoC_ACC_LCA_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'",
		    Modelname, PoC_ACC_LCA_DictDefines[i].Name);
	}
...
/*
 * PoC_ACC_LCA_SetParams() will be invoked indirectly by the generated
 * model C code each time PoC_ACC_LCA_New() is called.
 */
void
PoC_ACC_LCA_SetParams (rtModel_PoC_ACC_LCA *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
static void *
PoC_ACC_LCA_New (struct tInfos *Inf, const char *KindKey)
{
    rtModel_PoC_ACC_LCA *rtm;
    ...
    MatSupp_ResetQuants(PoC_ACC_LCA_DictDefines);
...
static int
PoC_ACC_LCA_Calc (void *MP, double dt)
{
    rtModel_PoC_ACC_LCA *rtm = (rtModel_PoC_ACC_LCA *)MP;
    int osCount=0;
    ExternalInputs_PoC_ACC_LCA  *in  = (ExternalInputs_PoC_ACC_LCA *) rtmGetU(rtm);
    ExternalOutputs_PoC_ACC_LCA *out = (ExternalOutputs_PoC_ACC_LCA *)rtmGetY(rtm);
...
PoC_ACC_LCA_Register (void)
{
    ...
    m.VehicleControl.DeclQuants		= PoC_ACC_LCA_DeclQuants;
    m.VehicleControl.New		= PoC_ACC_LCA_New;
    m.VehicleControl.Calc		= PoC_ACC_LCA_Calc;
    m.VehicleControl.Delete		= PoC_ACC_LCA_Delete;
